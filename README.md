...
Иконки **не хранятся** в репозитории — они генерируются локально. После клонирования перед первой сборкой `.dmg` выполни:
...
│   ├── icon.png         # 256px PNG (генерируется, не в git)
│   └── icon-512.png     # 512px PNG для DMG (генерируется, не в git)
...
Файл `accounts.json` с импортированными аккаунтами **привязан к этой машине**.
Перенос его на другой компьютер не сработает — секрет шифрования зависит от
платформы, домашней директории и имени пользователя.
При переносе импортируйте аккаунты заново на новой машине.
